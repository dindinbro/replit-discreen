...
function TelegramLookupPanel() {
  ...
  return (
    <motion.div
      ...
      style={{ borderColor: "rgba(34,158,217,0.22)", background: "rgba(34,158,217,0.025)" }}
    >
      {/* Header */}
      <div
        className="flex items-center gap-3 px-6 py-4 border-b"
        style={{ background: "rgba(34,158,217,0.07)", borderColor: "rgba(34,158,217,0.18)" }}
      >
        <div
          className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
          style={{ background: "rgba(34,158,217,0.13)", border: "1px solid rgba(34,158,217,0.25)" }}
        >
          <SiTelegram className="w-5 h-5" style={{ color: TG }} />
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-bold leading-tight">Telegram Lookup</h2>
          <p className="text-xs text-muted-foreground">Recherche dans la base de données Telegram indexée</p>
        </div>
        <Badge
          className="text-[10px] font-bold tracking-widest px-2.5 py-1 border"
          style={{
      ...
      <div className="p-6 md:p-8 space-y-7">
        ...
        <div className="flex gap-2">
          {(["id", "phone"] as const).map(m => (
            <button
              ...
              style={
                mode === m
                  ? { background: "rgba(34,158,217,0.14)", color: TG, borderColor: "rgba(34,158,217,0.35)", fontWeight: 600 }
                  : { color: "var(--muted-foreground)", borderColor: "rgba(34,158,217,0.12)" }
              }
              data-testid={`button-telegram-tab-${m}`}
            >
              {m === "id" ? "ID Telegram" : "Numéro de téléphone"}
            </button>
          ))}
        </div>
        ...
        {mode === "id" && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Hash className="w-4 h-4" style={{ color: TG }} />
              <label className="text-sm font-semibold">ID Telegram</label>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              />
              <div className="relative group" data-testid="button-telegram-search-id-wrap">
                <Button
                  disabled
                ...
                <span
                  className="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md px-2.5 py-1 text-[11px] font-bold tracking-widest pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity"
                  style={{ background: "rgba(34,158,217,0.18)", color: TG, border: "1px solid rgba(34,158,217,0.3)" }}
                >
                  BIENTÔT DISPONIBLE
                </span>
              </div>
        ...
        {mode === "phone" && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Phone className="w-4 h-4" style={{ color: TG }} />
              <label className="text-sm font-semibold">Numéro de téléphone</label>
            </div>
            <p className="text-xs text-muted-foreground">Retrouvez un compte Telegram associé à un numéro de téléphone</p>
            <div className="flex items-center gap-3 flex-wrap">
              <Input
                value={inputPhone}
              ...
              />
              <div className="relative group" data-testid="button-telegram-search-phone-wrap">
                <Button
                  disabled
                  className="gap-2 cursor-not-allowed"
                  style={{ opacity: 0.55 }}
        ...
        {/* Indexation notice */}
        <div
          className="flex items-center gap-3 rounded-xl px-5 py-3.5 text-sm border"
          style={{ background: "rgba(34,158,217,0.06)", borderColor: "rgba(34,158,217,0.15)" }}
        >
          <span className="w-2 h-2 rounded-full shrink-0 animate-pulse" style={{ background: TG }} />
          <span className="text-muted-foreground">
            Base en cours d'indexation — la recherche sera disponible prochainement.
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { title: "ID → Profil", desc: "Retrouvez un profil complet à partir de son identifiant unique Telegram" },
            { title: "Téléphone → Compte", desc: "Associez un numéro de téléphone à un compte Telegram" },
            { title: "Base indexée", desc: "Milliards d'entrées issues de groupes publics et de leaks Telegram" },
          ].map(({ title, desc }) => (
            <div
              key={title}
              className="rounded-xl p-4 space-y-1.5 border"
              style={{ background: "rgba(34,158,217,0.04)", borderColor: "rgba(34,158,217,0.12)" }}
            >
              <p className="font-semibold text-sm" style={{ color: TG }}>{title}</p>

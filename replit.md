...
- **Username OSINT** (`/search` → Username OSINT tab): Three-source username/email lookup: **Sherlock** (~50 popular sites by username), **WhatsMyName** (700+ sites by username via GitHub JSON, cached 24h), **Holehe** (~25 sites by email, checks signup/recovery endpoints). Each has its own backend endpoint (`/api/sherlock`, `/api/whatsmyname`, `/api/holehe`). VIP+ required. Holehe uses teal color scheme; Sherlock/WMN use purple.
...
- **Subscription & Usage Limits**: Implements a tiered subscription model with two billing modes: **monthly** (30-day expiry) and **lifetime** (expires 2099). Daily usage limits enforced atomically, with account freezing for administrators. A cron job runs every 5 minutes to automatically expire subscriptions. Freezing pauses the subscription timer. Frozen accounts excluded from auto-expiry. Lifetime prices: VIP €69.99, PRO €124.99, API €399.99. Lifetime keys prefixed `LFT-`, monthly keys prefixed `DSC-`. Order IDs encode billing: `order_vip_lifetime_...` vs `order_vip_...`. Free plan: 3 searches/day.
...
- **DaltonAPI**: External API with identical format to LeakOSINT (proxied via `POST /api/dalton-search`). Called in parallel with LeakOSINT during advanced search. Shares the same daily quota (LeakOSINT counts the usage; DaltonAPI only checks but doesn't increment). Secret: `DALTON_API_KEY`.
